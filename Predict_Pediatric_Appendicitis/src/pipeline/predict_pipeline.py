...
class PredictionPipeline:
    ...
    def save_input_files(self) -> str:
        try:
            ...
            logging.error(f"Error saving input files: {str(e)}")
    ...
    def get_predicted_dataframe(self, input_dataframe_path: str):
        try:
            ...
            logging.info(f"DataFrame shape before prediction: {input_dataframe.shape}")

            predictions = self.predict(input_dataframe)

            input_dataframe[prediction_column_name] = predictions

            # Update this mapping according to the Regensburg project
            target_column_mapping = {0: "no_appendicitis", 1: "appendicitis"}
            ...
            logging.info(
                f"Predictions saved to: {self.prediction_pipeline_config.prediction_file_path}"
            )
